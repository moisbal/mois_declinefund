...
const VERSION = '20260903000200';
const SQL_PATH = path.resolve(process.cwd(), 'supabase/migrations',
  `${VERSION}_new_project_pending_fund_path.sql`);
...
async function snapshot(client) {
  const result = await client.query(`select
    ...
    (select count(*)::text from public.financial_unallocated_fund_lots) lots,
    (select count(*)::text from public.financial_unallocated_fund_movements) movements`);
...
async function main() {
  ...
  const client = new Client({
    ...
    application_name: `new-project-pending-path-${action}`,
  });
  ...
  try {
    ...
    const lock = await client.query(
      "select pg_try_advisory_xact_lock(pg_catalog.hashtextextended('new-project-pending-path', 20260903000200)) locked",
    );
    ...
    const functions = await client.query(`select signature,
      ...
      'get_financial_new_project_funding_sources(integer,uuid)',
    ...
    const internal = await client.query(`select
      to_regprocedure('public.financial_apply_budget_change_to_pending_funds(uuid)') is not null present,
      has_function_privilege('authenticated',
        'public.financial_apply_budget_change_to_pending_funds(uuid)', 'EXECUTE') auth_execute`);
    if (!internal.rows[0]?.present || internal.rows[0]?.auth_execute) {
      fail('Internal apply RPC privilege postcondition failed.');
    }
