...
const VERSION = '20260826000900';
const SQL_PATH = path.resolve(process.cwd(), 'supabase/migrations', `${VERSION}_budget_change_apply_hotfix.sql`);
...
async function main() {
  ...
  const client = new Client({ connectionString: connectionString(databaseUrl), ssl: { rejectUnauthorized: false },
    connectionTimeoutMillis: 15000, application_name: `budget-apply-${action}` });
  ...
  try {
    ...
    const definition = await client.query("select pg_get_functiondef('public.financial_apply_budget_change_request(uuid)'::regprocedure) as value");
    const value = definition.rows[0]?.value.toLowerCase() ?? '';
    if (!value.includes('request_lines.request_id = v_request.id') || !value.includes('for update of request_lines')) {
      fail('APPLY hotfix postcondition failed.');
    }
