...
const VERSION = '20260903000100';
const SQL_PATH = path.resolve(process.cwd(), 'supabase/migrations',
  `${VERSION}_new_project_funding_source_workflow.sql`);
...
async function main() {
  ...
  const client = new Client({
    ...
    application_name: `new-project-funding-${action}`,
  });
  ...
  try {
    ...
    const lock = await client.query(
      "select pg_try_advisory_xact_lock(pg_catalog.hashtextextended('new-project-funding-workflow', 20260903000100)) locked",
    );
    ...
    const functions = await client.query(`select signature,
      ...
      'get_financial_attachable_new_project_drafts(uuid)',
      'financial_save_new_project_request_draft(uuid,uuid,integer,text,text,text,text,integer,integer,text,text,uuid,uuid,uuid,bigint,uuid)',
      'financial_submit_new_project_request(uuid)',
      'financial_test_uat_save_budget_change_request_with_drafts(uuid,uuid,jsonb,date,text,uuid,boolean)',
    ]]);
    if (functions.rows.some((row) => !row.present || !row.auth_execute || row.anon_execute)) {
      fail('RPC existence or privilege postcondition failed.');
    }
