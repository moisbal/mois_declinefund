...
const VERSION = '20260826000800';
const SQL_PATH = path.resolve(process.cwd(), 'supabase/migrations', `${VERSION}_budget_change_candidate_hotfix.sql`);
...
async function main() {
  ...
  const client = new Client({ connectionString: connectionString(databaseUrl), ssl: { rejectUnauthorized: false },
    connectionTimeoutMillis: 15000, application_name: `budget-candidate-${action}` });
  ...
  try {
    ...
    const definition = await client.query("select pg_get_functiondef('public.get_financial_budget_change_candidates(uuid,text,integer,boolean)'::regprocedure) as value");
    if (!definition.rows[0]?.value.toLowerCase().includes('order by 2 desc, 4, 3')) fail('Candidate hotfix postcondition failed.');
