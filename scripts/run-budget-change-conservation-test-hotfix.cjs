...
const VERSION = '20260827000100';
const SQL_PATH = path.resolve(process.cwd(), 'supabase/migrations', `${VERSION}_budget_change_conservation_rules_hotfix.sql`);
...
async function main() {
  ...
  const client = new Client({ connectionString: connectionString(databaseUrl), ssl: { rejectUnauthorized: false },
    connectionTimeoutMillis: 15000, application_name: `budget-conservation-${action}` });
  ...
  try {
    ...
    const definitions = await client.query(`select
      ...
      to_regprocedure('public.get_financial_budget_change_next_year_candidates(uuid,text)')::text as next_year,
      exists (select 1 from pg_trigger where tgname = 'financial_validate_budget_change_line_year' and not tgisinternal) as year_trigger,
      exists (select 1 from pg_trigger where tgname = 'financial_link_registered_next_year_project' and not tgisinternal) as registered_trigger`);
    const row = definitions.rows[0] ?? {};
    if (!row.position?.includes('projects.alloc') || !row.candidates?.includes('projects.year = v_anchor_year')
        || !row.next_year || !row.year_trigger || !row.registered_trigger) fail('Conservation hotfix postcondition failed.');
    ...
    process.stdout.write(`${JSON.stringify({ ok: true, target: 'TEST', action,
      transaction: action === 'validate' ? 'ROLLED_BACK' : 'COMMITTED', migration_version: VERSION,
      migration_sha256: sha, monetary_unchanged: true }, null, 2)}\n`);
  } finally {
