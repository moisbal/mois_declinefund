...
const VERSION = '20260827000200';
const SQL_PATH = path.resolve(process.cwd(), 'supabase/migrations', `${VERSION}_budget_change_year_search_and_increase_source.sql`);
...
async function main() {
  ...
  const client = new Client({ connectionString: connectionString(databaseUrl), ssl: { rejectUnauthorized: false },
    connectionTimeoutMillis: 15000, application_name: `budget-year-search-${action}` });
  ...
  try {
    ...
    const definitions = await client.query(`select
      ...
      pg_get_functiondef('public.financial_validate_budget_change_line_year()'::regprocedure) as line_guard,
      pg_get_functiondef('public.financial_validate_budget_change_request(uuid)'::regprocedure) as request_guard`);
    const row = definitions.rows[0] ?? {};
    if (!row.candidates?.includes('projects.year <= v_anchor_year')
        || !row.candidates?.includes('order by 2 desc, 4, 3')
        || !row.line_guard?.includes('INCREASE_TARGET')
        || !row.request_guard?.includes('v_line_count <> 1')) {
      fail('Year-search/increase-source hotfix postcondition failed.');
    }
