...
const VERSION = '20260903000600';
const SQL_PATH = path.resolve(process.cwd(), 'supabase/migrations/20260903000600_execution_status_reason.sql');
...
async function main() {
  ...
  const client = new Client({
    ...
    application_name: `execution-status-reason-${VERSION}-${action}`,
  });
  ...
  try {
    ...
    const lock = await client.query(
      `select pg_try_advisory_xact_lock(pg_catalog.hashtextextended('execution-status-reason-${VERSION}', ${VERSION})) locked`,
    );
    ...
    const postcondition = (await client.query(`select
      exists (select 1 from information_schema.columns
        where table_schema='public' and table_name='projects' and column_name='execution_status_reason') project_reason,
      not exists (select 1 from information_schema.columns
        where table_schema='public' and table_name='projects' and column_name='delay_reason') old_reason_removed,
      exists (select 1 from information_schema.columns
        where table_schema='public' and table_name='financial_new_project_requests'
          and column_name='execution_status_reason') request_reason,
      to_regprocedure('public.update_my_project_metadata_v4(uuid,text,text,integer,text,jsonb,uuid,uuid[],text,text,text,text[],text,text,boolean,text,text)') is not null project_save,
      to_regprocedure('public.financial_save_new_project_request_draft_v2(uuid,uuid,integer,text,text,text,text,integer,integer,text,text,uuid,uuid,uuid,bigint,uuid,text)') is not null draft_save,
      to_regprocedure('public.financial_test_uat_save_budget_change_request_complete_v2(uuid,uuid,jsonb,date,text,uuid,boolean)') is not null grouped_save,
      has_function_privilege('authenticated',
        'public.update_my_project_metadata_v4(uuid,text,text,integer,text,jsonb,uuid,uuid[],text,text,text,text[],text,text,boolean,text,text)', 'EXECUTE') project_save_execute,
      has_function_privilege('authenticated',
        'public.financial_save_new_project_request_draft_v2(uuid,uuid,integer,text,text,text,text,integer,integer,text,text,uuid,uuid,uuid,bigint,uuid,text)', 'EXECUTE') draft_save_execute`)).rows[0];
    if (!postcondition?.project_reason || !postcondition?.old_reason_removed
        || !postcondition?.request_reason || !postcondition?.project_save
        || !postcondition?.draft_save || !postcondition?.grouped_save
        || !postcondition?.project_save_execute || !postcondition?.draft_save_execute) {
      fail('Execution-status-reason migration postcondition failed.');
    }
